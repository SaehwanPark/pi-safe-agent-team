...
- **Round-ten release hardening**: external ACK proofs, generation-safe resource reuse and incarnation-aware `resource.inspect` freshness checks, exact-limit artifact admission, persisted state v2 migration with a tagged v0.2.3 fixture, the `bench:r10-production-soak` production-default benchmark, package tarball coverage, and release workflow gates.
...
- **Round-six recovery convergence**: reconnectable actors remain reserved/live for topology, child-capacity, shutdown, and quiescence checks; stale recovery fences complete descendant subtrees and pauses stale-agent reclamation after long timer gaps; reconnect validates its parent and capacity; revisioned coalesced messages require exact ID/revision acknowledgements; recovery notices, durable write quarantines, terminal mailbox retention, startup cleanup, durable-state reconciliation, and root receipt markers now converge across restart and host races.
- **Round-five audit hardening**: root and child deliveries require exact durable session receipts before broker ACKs; lifecycle and `message.send` operations replay safely by `operationId`; task completion rejects live descendants and completed prerequisites cannot reopen while dependents remain active; stale actors release runtime claims and expire reconnect reservations; and coalesced control notifications carry the latest lease/task state.
- **Round-four durability and liveness hardening**: child broker messages remain unacknowledged until their Pi session transcript accepts them; blocked children release mutable leases and waiters while retaining task ownership; explicit task reopening schedules one deterministic recovery wake; reconnect drains cursor-based inbox batches; abort-shaped provider errors participate in root descendant shutdown; deferred root deliveries keep quiescence closed until acknowledgement; journals checkpoint by count/size and on clean stop; malformed safety configuration fails closed; explicit `capacityGroup` identities coordinate route aliases; root generation holds a local route permit; detached worktree shell forms are rejected; and control-plane notifications use a reserved/coalesced mailbox lane.
- **Round-two model/runtime hardening**: classify terminal Pi outcomes (logical overflow, prefill/KV capacity, runtime memory pressure, transient exhaustion, compaction failure, and fatal provider errors); block children with durable bounded diagnostics instead of laundering exhausted turns into `ready`, allow one capacity-relief retry, and prevent blocked inbox wakes from looping until task recovery is explicit. Root logical turns now start idempotently across Pi retry/compaction continuations, root compaction participates in conservative quiescence, and degraded root context suppresses automatic model wakes while preserving durable deliveries.
- **Route-aware model capacity**: add provider/model route policies and a process-local FIFO arbiter shared by child generations and compaction, with explicit effective prefill budgets (`min(logical window, configured safe budget)`) for embedded context management. Local backends default to one heavy operation unless configured otherwise.
- **Bounded emergency draining and interop diagnostics**: `/agents stop --now` starts local child aborts before broker work, later stop requests escalate a shared drain, terminal child handles remove themselves from the live map, and canonical `pi-local-context-manager.embedded-context.v1` discovery falls back to the legacy alias while exposing bounded context diagnostics.
