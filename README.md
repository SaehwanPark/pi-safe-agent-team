...
4. **Durable Idempotency & Crash Recovery**: `agent.spawn` and `task.create` enforce unique per-actor `operationId` records committed to an append-only transaction journal (`events.jsonl`). Ambiguous network/timeout retries replay original responses safely without duplicating agents or tasks. Broker recovery fences complete descendant subtrees and keeps reconnectable capacity reserved until reattach or expiry.
...
- **Recovery and Cleanup**: A reconnectable actor remains part of topology, child capacity, and quiescence accounting. Recovery grace expires recursively for a stale subtree; a later runtime startup cleans only clean worktrees and matching child session directories when no live or recovering actor references them. Dirty worktrees remain available for inspection.
