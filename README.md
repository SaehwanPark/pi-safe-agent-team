...
4. **Durable Idempotency & Crash Recovery**: `agent.spawn`, `task.create`, and message ACKs enforce unique per-actor `operationId` records committed to an append-only transaction journal (`events.jsonl`). Ambiguous network/timeout retries replay original responses safely without duplicating agents, tasks, or acknowledgements. Broker recovery fences complete descendant subtrees, retains task ownership during reconnect grace, and keeps reconnectable capacity reserved until reattach or expiry.
