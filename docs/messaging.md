...
- ACKs may carry a stable per-recipient `operationId` (`ack:<recipient>:<messageId>:<revision>`). The broker journals the acknowledgement and appends a compact `(recipient,messageId,revision)` proof to the cold `ack-proofs.jsonl` store, so a lost ACK response remains provably retryable after both message retention and the ordinary idempotency window prune the full record; an unproven `MESSAGE_NOT_FOUND` is never treated as success. Proof history is independent of hot coordinator rollback snapshots;
