...
`shared` is the caller's project cwd. `worktree` requires a clean Git checkout, creates an isolated branch/worktree, and records the path/base commit/branch. v1 never guesses how to merge changes and never removes a dirty or committed worktree automatically. A later runtime startup may reclaim a clean-at-base worktree and child session directory after the actor's recovery grace has expired and no live or reconnectable actor references the path; dirty or uncertain artifacts remain for inspection.
