...
- broker restart: old live actors and their live descendants are marked failed/reconnectable as one recovery-fenced subtree, leases are released, pending clarification records survive, matching sessions can re-register once after parent/depth/capacity validation, and each waiting actor keeps its `maxTotalAgents` slot reserved until it reconnects or the bounded `reconnectGraceMs` expires (so newcomers cannot evict it forever). If the broker's maintenance timer was suspended for a liveness interval, its first resumed pass gives actors a heartbeat opportunity before stale reclamation;
...
`shared` is the caller's project cwd. `worktree` requires a clean Git checkout, creates an isolated branch/worktree, and records the path/base ref/branch. v1 never guesses how to merge changes and never removes a dirty worktree automatically. A later runtime startup may reclaim a clean worktree and child session directory after the actor's recovery grace has expired and no live or reconnectable actor references the path; dirty or uncertain artifacts remain for inspection.
